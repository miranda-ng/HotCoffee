...
Source: x86\skins\smileys\_flash\*; DestDir: {app}\skins\smileys\_flash; Flags: ignoreversion; Components: resources\smileys; AfterInstall: AddDetails;
...
var
    ...
    szFile: String;
begin
    if not IniKeyExists('{#AppNameSetup}', 'UninstallFilesCount', ExpandConstant('{app}\profiles\{#AppINI}')) then Exit;
    for i := GetIniInt('{#AppNameSetup}', 'UninstallFilesCount', 0, 0, 0, ExpandConstant('{app}\profiles\{#AppINI}')) downto 1 do
    begin
        szFile := GetIniString('{#AppNameSetup}', Format('UninstallFile%d', [i]), '', ExpandConstant('{app}\profiles\{#AppINI}'));
        DeleteFile(ExpandConstant(szFile));
        if Pos('{app}', szFile) = 1 then
            RemoveDir(ExtractFileDir(ExpandConstant(szFile)));
    end;
end;
        ...
        ssPostInstall:
            begin
                ...
                begin
                    ...
                    SetIniInt('{#AppNameSetup}', 'UninstallFilesCount', g_UninstallFilesHelper.Count, ExpandConstant('{app}\profiles\{#AppINI}'));
                    for i := 1 to g_UninstallFilesHelper.Count do
                        SetIniString('{#AppNameSetup}', Format('UninstallFile%d', [i]), g_UninstallFilesHelper[i - 1], ExpandConstant('{app}\profiles\{#AppINI}'));
                end;
            end;
    end;
end;
