...
function checkaudio(message) {
  ...
  if (!result) return 0;
   var id = Math.random();
   document.write("<div id=\""+id+"\"><a class=\"script\" onclick=\"javascript: showaudio('"+mes_temp+"','"+id+"'); \">[Прослушать аудио]</a></div>");
}
...
function showaudio(url,id) {
  ...
  hidetext = "<br/><a class=\"script\" onclick=\"javascript: hideaudio('"+url+"','"+id+"'); \">[Скрыть аудио]</a>";
  document.getElementById(id).innerHTML = template+hidetext;
}

function hideaudio(url,id) {
  text = "<a class=\"script\" onclick=\"javascript: showaudio('"+url+"','"+id+"'); \">[Прослушать аудио]</a>";
